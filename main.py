...
for comment in reddit.subreddit(SUBREDDITS).stream.comments(skip_existing=True):
    ...
    if amongusified in {comment.body, reddit_spoiler(comment.body)}:
        ...
        if DEBUG: print(' Comment not amogusified', file=log)
        continue
    print('Replying to comment:', comment.permalink, file=log)
    print('', amongusified, file=log)
    try:
        comment.reply(amongusified + FOOTER)
    except PRAWException:
        print(' Error replying:', file=log, flush=True)
        traceback.print_exc()
    finally:
